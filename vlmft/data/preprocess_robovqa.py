    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "avg_frames_per_sample": (
                np.mean(self.frames_per_sample) if self.frames_per_sample else 0
            ),
            "avg_qas_per_sample": (
                np.mean(self.qas_per_sample) if self.qas_per_sample else 0
            ),
...
def parse_qa_text(text: str, frame_start: int, frame_end: int) -> List[QAPair]:
    ...
    while i < len(parts):
        ...
        qa_match = re.search(
            r"(.*)Q:\s*(.+?)\s*A:\s*(.+?)(?=<task:|$)", content, re.DOTALL
        )
        ...
        if qa_match:
            ...
            full_question = (
                f"{context} {question_text}".strip() if context else question_text
            )
...
def verify_preprocessed(output_dir: Path, num_samples: int = 100) -> bool:
    ...
    for jsonl_rel_path, label in jsonl_files:
        ...
        sample_indices = rng.choice(
            total_lines, min(num_samples, total_lines), replace=False
        )
...
def main():
    ...
    tfrecord_files = sorted(
        list(data_dir.glob("**/*.tfrecord")) + list(data_dir.glob("**/*.tfrecord-*"))
    )
    if args.max_files:
    ...
    if args.workers == 1:
        ...
        for i, tfrecord_path in enumerate(tfrecord_files):
            logging.info(
                f"Processing [{i+1}/{len(tfrecord_files)}]: {tfrecord_path.name}"
            )
            ...
            if (i + 1) % 10 == 0:
                logging.info(
                    f"  Progress: {sum(s.total_samples for s in all_stats)} samples processed"
                )
    else:
        # Parallel processing
